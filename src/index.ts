...
function _getReplacement(replacementChar: unknown): _Replacement {
  if ((typeof replacementChar === "string") && (replacementChar.length === 1)) {
    ...
    try {
      const replacementBytes: _Ms932CharBytes = _encodeChar(replacementChar.codePointAt(0) as codepoint, true, [ 0x3F ]);
      return {
 ...
 */
function _encodeChar(codePoint: codepoint, exceptionFallback: boolean, replacementFallback: Readonly<_Ms932CharBytes>): _Ms932CharBytes {
  if (codePoint <= 0x80) {
    // 2.
  ...
  // 7.
  const pointer = _TABLE.get(codePoint);
  ...
  // 9.
  const lead = Math.trunc(pointer / 188);

  // 10.
  const leadOffset = (lead < 0x1F) ? 0x81 : 0xC1;

  // 11.
  const tail = pointer % 188;
...
const _TABLE = new Map<codepoint, number>([
  ...
  */
  ...
  [ 0x2178, 10724 ],
  [ 0x2179, 10725 ],
  // [ 0x2160, 10726 ], 1148 重複
  // [ 0x2161, 10727 ], 1149 重複
  // [ 0x2162, 10728 ], 1150 重複
  // [ 0x2163, 10729 ], 1151 重複
  // [ 0x2164, 10730 ], 1152 重複
  // [ 0x2165, 10731 ], 1153 重複
  // [ 0x2166, 10732 ], 1154 重複
  // [ 0x2167, 10733 ], 1155 重複
  // [ 0x2168, 10734 ], 1156 重複
  // [ 0x2169, 10735 ], 1157 重複
  // [ 0xFFE2, 10736 ], 137 重複
  [ 0xFFE4, 10737 ],
  [ 0xFF07, 10738 ],
  [ 0xFF02, 10739 ],
  // [ 0x3231, 10740 ], 1201 重複
  // [ 0x2116, 10741 ], 1193 重複
  // [ 0x2121, 10742 ], 1195 重複
  // [ 0x2235, 10743 ], 165 重複
  [ 0x7E8A, 10744 ],
  [ 0x891C, 10745 ],
  [ 0x9348, 10746 ],
  ...
  export class Encoder implements TextEncoder {
    ...
    encode(input = ""): Uint8Array {
      ...
      for (const c of input) {
        ...
        const bytes = _encodeChar(codePoint, this.fatal, this.#common._replacement.bytes);
        tmp[written] = bytes[0];
        written = written + 1;
        if (bytes.length > 1) {
